...
namespace Schedule.API.Controllers
{
    ...
    public class SchedulesController : ControllerBase
    {
        ...
        [HttpGet("daily")]
        public async Task<ActionResult<IEnumerable<ScheduleTask>>> GetDailySchedules (DateTime date)
        {
        ...
        public async Task<ActionResult<IEnumerable<ScheduleTask>>> GetDailySchedulesForUser(DateTime date, string userName)
        {
            // Get the start and end of the specific day
            var startOfDay = date.Date; // Start of the day at 00:00:00
            var endOfDay = startOfDay.AddDays(1).AddTicks(-1); // End of the day at 23:59:59.9999999
        ...
        [HttpGet("weekly")]
        public async Task<ActionResult<IEnumerable<ScheduleTask>>> GetWeeklySchedules(DateTime date)
        {
        ...
        public async Task<ActionResult<IEnumerable<ScheduleTask>>    > GetWeeklySchedules(DateTime date, string userName)
        {
            // 1) Find the Monday of the week that 'date' falls in
            int diff = (7 + ((int)date.DayOfWeek - (int)DayOfWeek.Monday)) % 7;
            var startOfWeek = date.Date.AddDays(-diff);
        ...
        [HttpGet("monthly")]
        public async Task<ActionResult<IEnumerable<ScheduleTask>>> GetMonthlySchedules(DateTime date)
        {
        ...
        public async Task<ActionResult<IEnumerable<ScheduleTask>>> GetMonthlySchedulesForUser(DateTime date, string userName)
        {
            // Determine the start and end dates for the given month
            var startOfMonth = new DateTime(date.Year, date.Month, 1);
            var endOfMonth = startOfMonth.AddMonths(1).AddDays(-1); // Last day of the month
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<ScheduleTask>> GetScheduleTask(int id)
        {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateScheduleTask(int id, [FromBody] ScheduleTask request)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> CreateScheduleTask([FromBody] ScheduleTask request)
        {
            ...
            try
            {
                ...
                _context.ScheduleTasks.Add(request);
                await _context.SaveChangesAsync();
